...
export default function PackagesSection() {
    const [selectedTier, setSelectedTier] = useLocalStorageState<PackageKey>("evTier", "standard");
        ...
        <section className="mx-auto w-full max-w-6xl px-4 pb-14">
            <div className="pt-2">
                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    <div>
                        <h2 className="text-2xl font-semibold tracking-tight md:text-4xl">EV Charger Installation Packages</h2>
                        <p className="mt-2 max-w-2xl text-sm text-white/75 md:text-base">
                            Transparent tiers based on distance, wall type, and panel capacity. Final pricing is confirmed after a quick
                            panel photo + site details.
                        </p>
                    </div>
                ...
                <div className="mt-8 hidden gap-5 md:grid md:grid-cols-3">
                    {PACKAGES.map((p) => (
                        <div key={p.key} className="flex h-full flex-col rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
                            <div>
                                <div className="text-base font-semibold">{p.title}</div>
                                <div className="mt-2 text-xl font-semibold text-brand-gold">{p.price}</div>
                                <p className="mt-3 text-sm leading-relaxed text-white/75">{p.note}</p>
                ...
                <div className="mt-6 md:hidden">
                    <PackageCardMobile selectedKey={selectedTier} onSelect={setSelectedTier} />
                </div>
            </div>
        </section>
    );

...
export default function PackageCardMobile({
  ...
  const selected = useMemo(
    () => PACKAGES.find((p) => p.key === selectedKey)!,
    [selectedKey],
  );
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur">
          ...
          return (
            <button
              ...
              className={[
                ...
                active
                  ? "bg-brand-gold/15 text-white border-brand-gold/40"
      ...
      {/* Content */}
      <div className="flex min-h-105 flex-col p-5">
        <div>
          <div className="text-base font-semibold">{selected.title}</div>
          <div className="mt-2 text-lg font-semibold text-brand-gold">
            {selected.price}
          </div>

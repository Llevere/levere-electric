...
export default function NavLinks() {
  ...
  return (
    <nav className="relative hidden md:flex">
      <div
        ref={navRef}
        className="flex items-center gap-6 lg:gap-10 whitespace-nowrap mt-2"
        ...
        {NAV_ITEMS.map((item, index) => {
          ...
          return (
            <Link
              ...
              className={`pb-3 text-[13px] lg:text-sm font-medium transition-colors ${
                active
                  ? "text-brand-gold"
      ...
      <span
        ref={indicatorRef}
        className="absolute bottom-5 h-0.5 bg-brand-gold transition-[width,transform,opacity] duration-300 ease-out"
      />
    </nav>
  );
}
